...
spark = SparkSession.builder.appName('ETL_CreateTransformedFactSalesOrderHeader').getOrCreate()
...
dbutils.widgets.text("sourceEntityName", "SalesOrderHeader")
dbutils.widgets.text("sourceSubjectID", "27")
dbutils.widgets.text("sourceEntityID", "15")

dbutils.widgets.text("targetSubjectName", "Sales_Trans")
dbutils.widgets.text("targetEntityName", "FactSalesOrderHeader")
dbutils.widgets.text("targetSubjectID", "28")
dbutils.widgets.text("targetEntityID", "16")

dbutils.widgets.text("dataHistoryID", "1")
dbutils.widgets.text("BusinessKeys", "")

dbutils.widgets.text("LoadType", "FullLoad")
dbutils.widgets.text("EntityType", "Fact")
...
transformedDf = spark.sql("""
                          ...
                          PurchaseOrderNumber,
                          AccountNumber,
                          CONCAT('CustomerID=', IFNULL(CustomerID, 0)) as FK_DimCustomer_ID,
                          SalesPersonID,
                          TerritoryID,
                          BillToAddressID,
                          ShipToAddressID,
                          ...
                          CONCAT('FromCurrencyCode=', IFNULL(dcr.FromCurrencyCode, ''), ';ToCurrencyCode=', IFNULL(dcr.ToCurrencyCode, '')) as FK_DimCurrencyRate_ID,
                          SubTotal,
                          TaxAmt,
                          Freight,
                          TotalDue,
                          Comment,
                          rowguid,
                          vtq.FK_ID_DataLoadHistory as DL_DataLoadHistory_ID,
                          vtq.ModifiedDate AS ValidFromDWLoadDate,
                          current_timestamp() as Transformed_CreatedTime,
                          '{0}' as TransformedSubject
                          FROM vw_transformedQuery  vtq
                          LEFT JOIN sales_trans.dimcurrencyrate dcr
                          ON vtq.CurrencyRateID = dcr.CurrencyRateID               
