...
spark = SparkSession.builder.appName('ETL_CreateTransformedDimCustomer').getOrCreate()
...
dbutils.widgets.text("sourceEntityID", "3")

dbutils.widgets.text("targetSubjectName", "Sales_Trans")
dbutils.widgets.text("targetEntityName", "DimCustomer")
dbutils.widgets.text("targetSubjectID", "28")
dbutils.widgets.text("targetEntityID", "4")

dbutils.widgets.text("dataHistoryID", "1")
dbutils.widgets.text("BusinessKeys", "CustomerID")
...
transformedDf = spark.sql("""
                          SELECT CustomerID, 
                          CONCAT('CustomerID=', CustomerID) AS BusinessKey,
                          PersonID,
                          StoreID,
                          TerritoryID,
                          AccountNumber,
