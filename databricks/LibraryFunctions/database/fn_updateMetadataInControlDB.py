...
def fn_updateMetadataInControlDB(sourceDf, entityID):
    ...
    check_metadata_query = f"(SELECT  [EntityID] as EntityID_df1, [ColumnName] as ColumnName_df1, [DataType] as DataType_df1, [MaxLength] as MaxLength_df1, [IsNullable] as IsNullable_df1, [Description] as [Description_df1] FROM [METADATA].[ColumnDetails] WHERE EntityID = {entityID} and IsActiveFlag = 1) as temp_alias"
    ...
    #If metadata records do not exist, then...
        ...
        try:
            ...
            queryStatement = f"UPDATE [METADATA].[ColumnDetails] SET [IsActiveFlag] = 0 WHERE [EntityID] == {entityID}"
            fn_execQueryInSQLControlDB(queryStatement)
            fn_appendDataToSQL(existing_metadata_records)
        ...
        try:
            ...
            existing_and_current_metadata_records = existing_and_current_metadata_records.withColumn('IsSchemaMatching', when((col('df1.DataType_df1') == col('df2.DataType')) & (col('df1.ColumnName_df1') == col('df2.ColumnName')) & ((col('df1.MaxLength_df1') == col('df2.MaxLength')) | col('df2.MaxLength').isNull() | col('df1.MaxLength_df1').isNull()) & (col('df1.IsNullable_df1')== col('df2.IsNullable'))  , True).otherwise(False))
            ...
            try:
                for row in existing_and_current_metadata_records.filter(col("IsSchemaMatching") == False).collect():
                    ...
                    if initialDataType != currDataType:
